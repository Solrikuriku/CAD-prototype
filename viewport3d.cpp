...
void Viewport3D::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if (m_rotate_mode == RotateMode::Rotate)
    {
        if (!m_transformObject) return;

        auto& objectPos = m_transformObject->transform.position;
        auto& objectRotate = m_transformObject->transform.rotation;
    ...
    else if (m_scale_mode == ScaleMode::Scale)
    {
        ...
        auto &scale = m_transformObject->transform.scaling;
...
void Viewport3D::keyPressEvent(QKeyEvent *event)
{
    ...
    if (m_rotate_mode == RotateMode::Choose)
    {
        ...
        m_cursorWorldPos = GetCursorWorldPos(mapFromGlobal(QCursor::pos()), m_transformObject->transform.position, m_currentView, m_axes);
...
SceneObject* Viewport3D::PickObject(const QPointF &currentPos)
{
